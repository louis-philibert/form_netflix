<?php 
	...
	if(!empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password_two'])){
		...
		if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
			header('location: inscription.php?error=1&message=Votre email est invalide.');
		}
		...
		while($email_verification = $req->fetch()){
			if($email_verification['numberEmail'] != 0){
				header('location: inscription.php?error=1&message=Votre adresse est déjà utilisée par un autre utilisateur.');
				exit();
			}
		}
	...
	<section>
		<div id="login-body">
			...
			<?php if(isset($_GET['error'])){
					...
					echo '<div class=" alert success">Vous étes desormais inscrit. <a href="index.php">Connectez-vous</a></div>';
			}
